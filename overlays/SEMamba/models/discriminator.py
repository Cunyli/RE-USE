...
def batch_pesq(clean, noisy, cfg):
    ...
    valid_mask = pesq_score != -1
    if not np.any(valid_mask):
        return None, torch.BoolTensor(valid_mask)
    pesq_score = (pesq_score[valid_mask] - 1) / 3.5
    return torch.FloatTensor(pesq_score), torch.BoolTensor(valid_mask)
